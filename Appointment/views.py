...
from user.models import User, Kids,WeekTimeSlots
...
def get_common_slots(class_instructor, start_date, profile_user):
    ...
    for day in day_list:
        ...
        for start_time in time_list[len(time_list)-1::-1]:
...
class GetDateTimeSlots(APIView):
    def get(self, request):
        ...
        if serializer.is_valid():
            ...
            # print("ye dict hai",{'date': day_result, 'time': time_result})
            response=Response({'date': day_result, 'time': time_result})
            print("response data hai ye",response.data)
    ...
    def post(self, request):
        ...
        if appointments:
            for appointment in appointments:
                ...
                print("my dates and time", appointment.start_time, appointment.end_time)
...
class AppointmentScheduleViewSet(APIView):
    ...
    def get(self, request):
        try:
            ...
            us_now = datetime.now(tz=us_eastern)
            if appointments:
                print("previous appoiment",datetime.now())
                prev_appointment = AppointmentScheduleSerializer(
                   
                    appointments.filter(start_time__lt=us_now).order_by('-start_time'), many=True,
                ...
                if prev_appointment.data or next_appointment.data:
                    ...
                    # print("prev_appointment.data",prev_appointment.data)
                    # print("prev_appointment.data.start_time",prev_appointment.data.start_time)
                    print( {"user_details": obj, 'prev_session': prev_appointment.data, 'kid_detail': kid_detail,
