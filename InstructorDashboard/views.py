...
def signup_view(request):
    ...
    if request.method == 'GET':
        ...
        if user_models.User.objects.filter(email=email).exists():
            ...
            return redirect("/register?type=register")
...
def dashboard_view(request):
    ...
    count=0
    for stu in student_count:
        if not stu.parent.deactivate:
            count=count+1
...
def user_pass(request):
    ...
    return redirect('/')
def deactivate_trainee_view(request,id):
    student_data=Kids.objects.get(pk=id)
    student_data.parent.deactivate=True
    student_data.parent.save()
    return redirect('InstructorDashboard:students')
