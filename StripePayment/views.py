    ...
    def get(self, request, id):
        if appointment_model.Transaction.objects.filter(id=id).exists():
            ...
            return render(request, "payment_detail.html",
                          {"data": res_data, "user_details": user_details, "kid_detail": kid_detail})
        else:
        ...
        if request.method == "POST":
                ...
                if data["status"] == "succeeded":
                    ...
                    if paid_amount > due_amount:
                        ...
                        return redirect("dashboard_view")
                    due_amount=0
                    obj= appointment_model.Transaction.objects.filter(booking=booking.id)
                    if obj:
                        for amt in obj:
                            due_amount += amt.paid_amount 
                    ...
                    transaction_obj = appointment_model.Transaction.objects.create(
                        ...
                        due_amount=(int(booking.class_instructor.price - int(due_amount))) - int(paid_amount))
                    ...
                    try:
                        ...
                        user_email = booking.user.email
                        appoinment=appointment_model.Appointment.objects.filter(booking=booking)
                        ...
                        if count == 1:
                            # date_str = appoinment.start_time
                            l=[]
                            for i in appoinment:
                                time = i.start_time
                            ...
                            email_body = f"Dear {user_name}," \
                                         ...
                                         f"Your Appointment Schedule At -\n{Schedule1}\n\n" \
                                         f"Thank You,\nSwim Time Solutions"
                        ...
                        #                  f"\n\nThis mail is to inform you that you have made payment of {paid_amount}
                        #                  USD.\n\n" \
        ...
        if 'email' in request.session:
            if serializer.validated_data['payment_type'] == '2':
                ...
                if count == 1:
                    ...
                    # date=date_str.strftime('%Y-%m-%d %I:%M:%S %p')
                    l=[]
                    for i in appoinment:
                        time = i.start_time
                        date = time.strftime('%m/%d/%Y %I:%M:%S %p')
        ...
        try:
            if 'email' in request.session:
                ...
                return render(request, "payment.html",
                              {"data": ser.data, "profile_detail": profile_detail, "user_details": obj,
                               'kid_detail': kid_detail, "BASE_URL": BASE_URL})
